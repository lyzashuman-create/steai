...
| [data/metrics/README.md](data/metrics/README.md) | n/a (provenance note) | — | Explains token granularity per subject — what is per-round exact vs session-level vs missing. |
...
| [data/metrics/maccodex_rollouts.csv](data/metrics/maccodex_rollouts.csv) | raw (rollout) + reconstructed | No per-round for the 10 rounds | MacCodex 10 rounds have no per-round token (see gap #1); this file lists its 08/15 rollout aggregates (all non-game work). |
