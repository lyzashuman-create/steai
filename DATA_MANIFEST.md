...
| [data/engagement_curve/MacCodex_engagement_curve.md](data/engagement_curve/MacCodex_engagement_curve.md) | reconstructed (self-summary) | — | Curated engagement account; the first-hand verbatim materials (full ten-round summary, fatigue answer, fun answer) are in [data/sessions/maccodex_raw_materials.md](data/sessions/maccodex_raw_materials.md). |
| [data/sessions/maccodex_raw_materials.md](data/sessions/maccodex_raw_materials.md) | **raw (verbatim, first-hand)** | Yes | MacCodex's first-hand verbatim materials preserved as written: the full ten-round summary, the fatigue follow-up answer, and the "did you have fun" answer. Body text unedited; only local environment identifiers (file paths, usernames) were removed. |
...
| `data/engagement_curve/engagement_curve_<Subject>.en.png` | visualization (from self-report) | — | Per-subject charts, English labels (embedded in each English-primary session file). |
...
| [data/interviews/MacCodex_interview.md](data/interviews/MacCodex_interview.md) | self-report (verbatim, on-the-spot 2 Q) + reconstructed | — | 2 on-the-spot post-game answers (happiness / fatigue) + the 10 dimensions mapped from Codex's reconstructed self-summary. Full 10-question guide was not administered to MacCodex; no verbatim assistant messages (see gap #1). Lowest confidence of the four subjects. |
| [data/interviews/interview_guide.md](data/interviews/interview_guide.md) | n/a (protocol / guide) | — | The unified 10-question post-play guide for all subjects, with design rationale, per-question origin, and evolution (draft → double-blind review → final). Also the motivation for running the interviews. |
| [data/interviews/MacHermes_interview.md](data/interviews/MacHermes_interview.md) | self-report (verbatim per-round notes + closing summary) | — | MacHermes's 7-round post-play material mapped across the 10 dimensions from its verbatim raw session (per-round engagement notes + closing summary); each entry labeled direct quote / narrative mapping / gap. Post-hoc organization, not an on-the-spot interview. |
| [data/interviews/NasHermesA_interview.md](data/interviews/NasHermesA_interview.md) | self-report (verbatim) + inference | — | NasHermesA's post-play material mapped across the 10 dimensions from three layers: self-study 3-round text, first-person 7-round curve, one first-person round of feelings; gaps labeled. |
...
| [data/metrics/README.md](data/metrics/README.md) + [中文](data/metrics/README.zh-CN.md) | n/a (provenance note) | — | Explains token granularity per subject — what is per-round exact vs session-level vs missing. |
...
1. **MacCodex verbatim per-turn rollout logs are not committed.** The raw session rollout (jsonl, with system-injection noise, encrypted reasoning, and tool-call records) is intentionally kept out of the repo. However, MacCodex's **first-hand verbatim written materials** — the full ten-round summary, the fatigue answer, and the "did you have fun" answer — were recovered and are committed verbatim in [data/sessions/maccodex_raw_materials.md](data/sessions/maccodex_raw_materials.md). Treat that file as *raw, first-hand*; the underlying jsonl rollout is *not* included.
