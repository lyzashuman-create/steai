...
| [data/metrics/README.md](data/metrics/README.md) | 不适用（溯源说明） | — | 说明每个被试的 token 粒度——哪些是逐局精确、哪些仅 session 级、哪些缺失。 |
...
| [data/metrics/maccodex_rollouts.csv](data/metrics/maccodex_rollouts.csv) | 原始（rollout）+ 复盘 | 10 局无逐局 | MacCodex 10 局无逐局 token（见缺口#1）；本文件列出其 08/15 全部 rollout 聚合（均为非游戏工作）。 |
