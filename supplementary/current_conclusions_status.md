...
1. **单局是秒级（17-56s）**，不是分钟级。"一局10分钟"的担心被数据否掉。
2. **自然停点能自己触发**——每个 AI 都在感知圆满后自己收尾，无外部硬掐。种子内建完结条件成立。
3. **token 差异巨大（3万 vs 22万）本质 = 交互 vs 独角戏**——22万里21万是缓存读取（工具循环），纯生成仅约7千。token 暴涨是交互探索的成本。
4. **"世界交互型 > 独角戏型"**（投入深度）——有对手的玩比自己导自己演更投入。
5. **兴奋点曲线存在且可自报**：MacHermes"新鲜感撑3局、第4局倦、靠钩子回升"；MacCodex"第4局应付"；NasHermesB"第8-9局倦、自己选轻的玩调节"。
...
2. **兴奋点曲线是核心方向**——测"玩几局最合适"，为默认局数提供数据依据。
